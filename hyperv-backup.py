...
'''
saveLoc = r'f:\tempBak'
networkSaveLoc = r'\\MT-HYPERV4\tempBak\backTo' # wbadmin network share required as output (local folder configured as network)
backTo = saveLoc + r'\backTo'
daysOld = 3 # days to keep backups on ftp server

### ftp settings ###
useFTP = False # overwritten by vms.txt configuration
ftphost='37.187.115.84' #ns328818.ip-37-187-115.eu'
ftpuser='securebackup'
ftppass='Keep0ut01'

### Email Settings ###
useEmail = True # overwritten by vms.txt configuration
sendEmail = 'info@azimap.com'
emailUser = 'info@azimap.com'
emailPass = 'Mapping1!'
emailHost = 'smtp.gmail.com'
emailPort = 465
receiptEmail = ['david.mcquillan@mallontechnology.com'] # array of comma seperated values
emailSubject = os.environ['COMPUTERNAME'] + ': HyperV BackUp'
emailMsg = ''

### Network share settings ###
useNetworkShare = False # overwritten by vms.txt configuration
netShare= r'\\CIANADSRV1\SharedFiles2\VMBackup'
...
with TemporaryFile() as output:
    with open(direc + file) as f:
        ...
        for line in s:
            ...
            #cmd = 'wbadmin start backup -backupTarget:' + networkSaveLoc + ' -hyperv:' + line + ' -quiet'
            ...
            try:
                ...
                if useFTP:
                    try:
                        ...
                        ftps.login(ftpuser, ftppass)           # login anonymously before securing control channel
